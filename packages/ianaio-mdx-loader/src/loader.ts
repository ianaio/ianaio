...
import type {MarkdownConfig} from '@ianaio/types';
...
function createAssetsExportCode(assets: unknown) {
  ...
  function createAssetValueCode(assetValue: unknown): string | undefined {
    ...
    if (typeof assetValue === 'string' && assetValue.startsWith('./')) {
      ...
      // Probably not worth adding more support, as we want to move to Webpack 5 new asset system (https://github.com/ianaio/ianaio/pull/4708)
...
// see https://github.com/ianaio/ianaio/issues/8298
function ensureMarkdownConfig(reqOptions: Options) {
  if (!reqOptions.markdownConfig) {
    throw new Error(
      'Ianaio v3+ requires MDX loader options.markdownConfig - plugin authors using the MDX loader should make sure to provide that option',
    );
  }
}
...
export async function mdxLoader(
  ...
  if (isMDXPartial && hasFrontMatter) {
    const errorMessage = `Ianaio MDX partial files should not contain front matter.
