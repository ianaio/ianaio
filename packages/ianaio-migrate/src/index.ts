...
import {Globby, IANAIO-VERSION} from '@ianaio/utils';
...
export async function migrateIanaioProject(
  siteDir: string,
  newDir: string,
  ...
  try {
    await fs.outputFile(
      path.join(newDir, 'ianaio.config.js'),
...
>): VersionTwoConfig {
  ...
  logger.info`Following Fields from path=${'siteConfig.js'} will be added to path=${'ianaio.config.js'} in code=${'customFields'}: ${Object.keys(
    customConfigFields,
  )}`;
...
function createClientRedirects(context: MigrationContext): void {
  if (!context.v1Config.cleanUrl) {
    context.deps['@ianaio/plugin-client-redirects'] = IANAIO-VERSION;
...
async function migratePackageFile(context: MigrationContext): Promise<void> {
  ...
  packageFile.scripts = {
    ...
    start: 'ianaio start',
    build: 'ianaio build',
    swizzle: 'ianaio swizzle',
    deploy: 'ianaio deploy',
    ianaio: 'ianaio',
  };
  if (packageFile.dependencies) {
    delete packageFile.dependencies.ianaio;
  }
  if (packageFile.devDependencies) {
    delete packageFile.devDependencies.ianaio;
  }
