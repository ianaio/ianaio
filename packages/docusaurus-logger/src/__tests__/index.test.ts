...
describe('interpolate', () => {
  ...
  it('recognizes valid flags', () => {
    expect(
      logger.interpolate`(keepAnsi) The package at path=${'packages/ianaio'} has number=${10} files. name=${'Babel'} is exported here subdue=${'(as a preset)'} that you can with code=${"require.resolve('@ianaio/core/lib/babel/preset')"}`,
    ).toMatchInlineSnapshot(
      `"(keepAnsi) The package at <cyan><underline>"packages/ianaio"</underline></color> has <yellow>10</color> files. <blue><bold>Babel</intensity></color> is exported here <gray>(as a preset)</color> that you can with <cyan>\`require.resolve('@ianaio/core/lib/babel/preset')\`</color>"`,
    );
  });
  it('interpolates arrays with flags', () => {
    expect(
      ...
      - <cyan>\`ianaio start\`</color>
      - <cyan>\`ianaio build\`</color>
      - <cyan>\`ianaio deploy\`</color>"
    `);
  });
  ...
  it('throws with bad flags', () => {
    expect(
      ...
      `"Bad Ianaio logging message. This is likely an internal bug, please report it."`,
    );
  });
});
