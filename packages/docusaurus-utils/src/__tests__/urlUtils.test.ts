...
describe('getEditUrl', () => {
  it('returns right path', () => {
    expect(
      getEditUrl('foo/bar.md', 'https://github.com/ianaio/ianaio'),
    ).toBe('https://github.com/ianaio/ianaio/foo/bar.md');
    expect(
      getEditUrl('foo/你好.md', 'https://github.com/ianaio/ianaio'),
    ).toBe('https://github.com/ianaio/ianaio/foo/你好.md');
  });
...
describe('buildSshUrl', () => {
  it('builds a normal ssh url', () => {
    const url = buildSshUrl('github.com', 'ianaio', 'ianaio');
    expect(url).toBe('git@github.com:ianaio/ianaio.git');
  });
  it('builds a ssh url with port', () => {
    const url = buildSshUrl('github.com', 'ianaio', 'ianaio', '422');
    expect(url).toBe('ssh://git@github.com:422/ianaio/ianaio.git');
  });
});
...
describe('buildHttpsUrl', () => {
  it('builds a normal http url', () => {
    ...
    expect(url).toBe('https://user:pass@github.com/ianaio/ianaio.git');
  });
  it('builds a normal http url with port', () => {
    ...
    expect(url).toBe(
      'https://user:pass@github.com:5433/ianaio/ianaio.git',
    );
  });
});
...
describe('hasSSHProtocol', () => {
  it('recognizes explicit SSH protocol', () => {
    const url = 'ssh://git@github.com:422/ianaio/ianaio.git';
    expect(hasSSHProtocol(url)).toBe(true);
  });

  it('recognizes implied SSH protocol', () => {
    const url = 'git@github.com:ianaio/ianaio.git';
    expect(hasSSHProtocol(url)).toBe(true);
  });

  it('does not recognize HTTPS with credentials', () => {
    const url = 'https://user:pass@github.com/ianaio/ianaio.git';
    expect(hasSSHProtocol(url)).toBe(false);
  });

  it('does not recognize plain HTTPS URL', () => {
    const url = 'https://github.com:5433/ianaio/ianaio.git';
    expect(hasSSHProtocol(url)).toBe(false);
  });
});
