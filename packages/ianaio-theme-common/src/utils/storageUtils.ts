...
function dispatchChangeEvent({
  ...
  // See https://github.com/ianaio/ianaio/issues/8594
...
/**
 * Will return `null` if browser storage is unavailable (like running Ianaio
 * in an iframe). This should NOT be called in SSR.
 *
 * @see https://github.com/ianaio/ianaio/pull/4501
 */
function getBrowserStorage(
  ...
  if (typeof window === 'undefined') {
    throw new Error(
      'Browser storage is not available on Node.js/Ianaio SSR process.',
    );
  }
  if (storageType === 'none') {
    return null;
...
function logOnceBrowserStorageNotAvailableWarning(error: Error) {
  if (!hasLoggedBrowserStorageNotAvailableWarning) {
    console.warn(
      `Ianaio browser storage is not available.
Possible reasons: running Ianaio in an iframe, in an incognito browser session, or using too strict browser privacy settings.`,
      error,
    );
...
Ianaio storage APIs are not supposed to be called on the server-rendering process.
 ...
 * this API can be a no-op. See also https://github.com/ianaio/ianaio/issues/6036
 */
  ...
  return {
    get: () => {
      try {
        ...
        console.error(`Ianaio storage error, can't get key=${key}`, err);
        return null;
      }
    },
    set: (newValue) => {
      try {
        ...
        console.error(
          `Ianaio storage error, can't set ${key}=${newValue}`,
          err,
        );
      }
    },
    del: () => {
      try {
        ...
        console.error(`Ianaio storage error, can't delete key=${key}`, err);
      }
    },
    listen: (onChange) => {
      try {
        ...
        console.error(
          `Ianaio storage error, can't listen for changes of key=${key}`,
          err,
        );
        return () => {};
      }
