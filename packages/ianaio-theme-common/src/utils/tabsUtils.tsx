...
import {useHistory} from '@ianaio/router';
import useIsomorphicLayoutEffect from '@ianaio/useIsomorphicLayoutEffect';
import {useQueryStringValue} from '@ianaio/theme-common/internal';
...
function ensureNoDuplicateValue(values: readonly TabValue[]) {
  ...
  if (dup.length > 0) {
    throw new Error(
      `Ianaio error: Duplicate values "${dup
        .map((a) => a.value)
...
function getInitialStateValue({
  ...
  if (tabValues.length === 0) {
    throw new Error(
      'Ianaio error: the <Tabs> component requires at least one <TabItem> children component',
    );
  }
  if (defaultValue) {
    ...
    if (!isValidValue({value: defaultValue, tabValues})) {
      throw new Error(
        `Ianaio error: The <Tabs> has a defaultValue "${defaultValue}" but none of its children has the corresponding value. Available values are: ${tabValues
          .map((a) => a.value)
          .join(
            ', ',
...
function getQueryStringKey({
  ...
  if (queryString === true && !groupId) {
    throw new Error(
      `Ianaio error: The <Tabs> component groupId prop is required if queryString=true, because this value is used as the search param name. You can also provide an explicit value such as queryString="my-search-param".`,
    );
  }
  return groupId ?? null;
}
...
export function useTabs(props: TabsProps): {
  ...
  // See https://github.com/ianaio/ianaio/issues/8625
