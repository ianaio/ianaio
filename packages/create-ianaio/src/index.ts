...
import {escapeShellArg} from '@ianaio/utils';
  ...
  if (typescript) {
    await fs.copy(template.path, dest, {
      filter: async (filePath) =>
        ...
        path.basename(filePath) === 'ianaio.config.js',
    });
  }
...
export default async function init(
  ...
  logger.info('Creating new Ianaio project...');
  ...
  if (source.type === 'git') {
    ...
    try {
      ...
      logger.error`Copying Ianaio template name=${source.template.name} failed!`;
      throw err;
    }
  } else {
    try {
