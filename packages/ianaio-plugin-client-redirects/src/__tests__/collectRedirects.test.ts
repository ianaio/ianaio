...
import {removeTrailingSlash} from '@ianaio/utils';
...
function createTestPluginContext(
  ...
  siteConfig: Partial<IanaioConfig> = {},
): PluginContext {
  return {
    outDir: '/tmp',
...
describe('collectRedirects', () => {
  ...
  it('throws when creating duplicate redirect routes and onDuplicateRoutes=throw', () => {
    expect(() =>
      ...
      "@ianaio/plugin-client-redirects: multiple redirects are created with the same "from" pathname: "/random-path"
    ...
    expect(() =>
      ...
      "@ianaio/plugin-client-redirects: some redirects would override existing paths, and will be ignored:
      - {"from":"/path-two","to":"/path-one"}"
    `);
  });
});
