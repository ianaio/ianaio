...
import {posixPath, getFileCommitDate} from '@ianaio/utils';
...
import type {
  ...
  PluginOptions,
  EditUrlFunction,
} from '@ianaio/plugin-content-blog';
...
const getPlugin = async (
  ...
  const generatedFilesDir: string = path.resolve(siteDir, '.ianaio');
...
describe('blog plugin', () => {
  it('getPathsToWatch returns right files', async () => {
    ...
    expect(relativePathsToWatch).toEqual([
      'i18n/en/ianaio-plugin-content-blog/authors.yml',
      'i18n/en/ianaio-plugin-content-blog/**/*.{md,mdx}',
      'blog/**/*.{md,mdx}',
    ]);
  });
  ...
  it('builds a simple website', async () => {
    ...
    expect(
      ...
      source: path.posix.join(
        '@site',
        path.posix.join('i18n', 'en', 'ianaio-plugin-content-blog'),
  ...
  it('handles edit URL with editLocalizedBlogs: true', async () => {
    ...
    expect(localizedBlogPost.metadata.editUrl).toBe(
      `${BaseEditUrl}/i18n/en/ianaio-plugin-content-blog/2018-12-14-Happy-First-Birthday-Slash.md`,
    );
  });
  ...
  it('handles edit URL with editUrl function', async () => {
    ...
    expect(editUrlFunction).toHaveBeenCalledWith({
      blogDirPath: 'i18n/en/ianaio-plugin-content-blog',
