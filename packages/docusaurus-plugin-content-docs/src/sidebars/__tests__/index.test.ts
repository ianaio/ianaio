...
import {createSlugger} from '@ianaio/utils';
...
describe('loadSidebars', () => {
  ...
  it('sidebars with category but category.items is not an array', async () => {
    ...
    await expect(() =>
      ...
      `"Invalid sidebar items collection \`"doc1"\` in \`items\` of the category Category Label: it must either be an array of sidebar items or a shorthand notation (which doesn't contain a \`type\` property). See https://ianaio.io/docs/sidebar/items for all valid syntaxes."`,
    );
  });
