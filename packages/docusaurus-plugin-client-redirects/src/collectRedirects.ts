...
import {addTrailingSlash, removeTrailingSlash} from '@ianaio/utils';
...
`,
  ...
  const toPaths = redirects
    ...
    // See https://github.com/ianaio/ianaio/issues/6845
    .map((to) => {
      if (to.startsWith('/')) {
        try {
  ...
  Object.entries(_.groupBy(redirects, (redirect) => redirect.from)).forEach(
    ([from, groupedFromRedirects]) => {
      if (groupedFromRedirects.length > 1) {
        ...
        )`name=${'@ianaio/plugin-client-redirects'}: multiple redirects are created with the same "from" pathname: path=${from}
It is not possible to redirect the same pathname to multiple destinations:${groupedFromRedirects.map(
  ...
  if (redirectsOverridingExistingPath.length > 0) {
    logger.report(
      pluginContext.siteConfig.onDuplicateRoutes,
    )`name=${'@ianaio/plugin-client-redirects'}: some redirects would override existing paths, and will be ignored:${redirectsOverridingExistingPath.map(
      (r) => JSON.stringify(r),
    )}`;
  }
  return newRedirects;
