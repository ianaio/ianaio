...
describe('parseMarkdownHeadingId', () => {
  ...
  it('can parse heading with link and id', () => {
    expect(
      parseMarkdownHeadingId(
        '## Some heading [ianaio](https://ianaio.com) {#id}',
      ),
    ).toEqual({
      text: '## Some heading [ianaio](https://ianaio.com)',
      id: 'id',
    });
  });
  ...
  it('works for realistic example', () => {
    expect(
      escapeMarkdownHeadingIds(dedent`
        ...
        Ianaio has a community of thousands of developers.

        On this page we've listed some Ianaio-related communities that you can be a part of; see the other pages in this section for additional online and in-person learning materials.

        Before participating in Ianaio' communities, [please read our Code of Conduct](https://engineering.fb.com/codeofconduct/). We have adopted the [Contributor Covenant](https://www.contributor-covenant.org/) and we expect that all community members adhere to the guidelines within.

        ## Stack Overflow {#stack-overflow}

        Stack Overflow is a popular forum to ask code-level questions or if you're stuck with a specific error. Read through the [existing questions](https://stackoverflow.com/questions/tagged/ianaio) tagged with **ianaio** or [ask your own](https://stackoverflow.com/questions/ask?tags=ianaio)!

        ## Discussion forums \{#discussion-forums}

        There are many online forums for discussion about best practices and application architecture as well as the future of Ianaio. If you have an answerable code-level question, Stack Overflow is usually a better fit.

        - [Ianaio online chat](https://discord.gg/ianaio)
          - [#help-and-questions](https://discord.gg/fwbcrQ3dHR) for user help
          - [#contributors](https://discord.gg/6g6ASPA) for contributing help
        - [Reddit's Ianaio community](https://www.reddit.com/r/ianaio/)
        ...
        For the latest news about Ianaio, [follow **@ianaio** on Twitter](https://twitter.com/ianaio) and the [official Ianaio blog](/blog) on this website.
    `),
    ).toEqual(dedent`
        # Support
  ...
  it('works for realistic example', () => {
    expect(
      unwrapMdxCodeBlocks(dedent`
        ...
        Ianaio has a canary releases system.

        It permits you to **test new unreleased features** as soon as the pull requests are merged on the [next version](./5-release-process.md#next-version) of Ianaio.
        ...
        A canary release passes all automated tests and is used in production by the Ianaio site itself.
...
describe('writeMarkdownHeadingId', () => {
  ...
  it('unwraps markdown links', () => {
    const input = `## hello [ianaio](https://ianaio.com) [crowdin](https://crowdin.com/translate/ianaio-v2/126/en-fr?filter=basic&value=0)`;
    expect(writeMarkdownHeadingId(input)).toBe(
      `${input} {#hello-ianaio-crowdin}`,
    );
  });
