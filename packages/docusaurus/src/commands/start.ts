...
import {normalizeUrl, posixPath} from '@ianaio/utils';
...
export async function start(
  ...
  // See https://github.com/ianaio/ianaio/issues/4542
  process.env.IANAIO_CURRENT_LOCALE = cliOptions.locale;
  ...
  logger.success`Ianaio website is running at: url=${openUrl}`;
    ...
    loadSite()
      .then(({baseUrl: newBaseUrl}) => {
        ...
        if (newOpenUrl !== openUrl) {
          logger.success`Ianaio website is running at: url=${newOpenUrl}`;
        }
      })
      .catch((err: Error) => {
  ...
  let config: webpack.Configuration = merge(
    await createClientConfig(props, cliOptions.minify, false),
    {
      ...
      infrastructureLogging: {
        // Reduce log verbosity, see https://github.com/ianaio/ianaio/pull/5420#issuecomment-906613105
        level: 'warn',
      },
      plugins: [
