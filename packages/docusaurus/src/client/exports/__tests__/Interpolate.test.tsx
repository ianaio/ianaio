...
describe('<Interpolate>', () => {
  ...
  it('rejects when children is not string', () => {
    expect(() =>
      ...
      `"The Ianaio <Interpolate> component only accept simple string values. Received: React element"`,
    );
    expect(() =>
      // @ts-expect-error: test
      renderer.create(<Interpolate>{null}</Interpolate>),
    ).toThrowErrorMatchingInlineSnapshot(
      `"The Ianaio <Interpolate> component only accept simple string values. Received: object"`,
    );
  });
});
