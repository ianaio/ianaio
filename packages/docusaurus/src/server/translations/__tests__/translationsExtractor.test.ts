...
import {SRC_DIR_NAME} from '@ianaio/utils';
import {
...
import type {InitializedPlugin, LoadedPlugin} from '@ianaio/types';
...
`,
    ...
    expect(errorMock).toHaveBeenCalledWith(
      expect.stringMatching(
        /Error while attempting to extract Ianaio translations from source code file at/,
      ),
    );
  });
...
`,
  ...
  it('does not extract from functions that is not ianaio provided', async () => {
    const {sourceCodeFilePath} = await createTmpSourceCodeFile({
      extension: 'js',
      content: `
...
`,
  ...
  it('recognizes aliased imports', async () => {
    const {sourceCodeFilePath} = await createTmpSourceCodeFile({
      extension: 'js',
      content: `
import Foo, {translate as bar} from '@ianaio/Translate';
...
`,
  ...
  it('recognizes aliased imports as string literal', async () => {
    const {sourceCodeFilePath} = await createTmpSourceCodeFile({
      extension: 'js',
      content: `
import {'translate' as bar} from '@ianaio/Translate';
