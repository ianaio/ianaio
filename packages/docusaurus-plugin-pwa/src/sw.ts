...
function parseSwParams() {
  ...
  if (params.debug) {
    console.log('[Ianaio-PWA][SW]: Service Worker params:', params);
  }
  return params;
}
...
async function runSWCustomCode(params: {offlineMode: boolean; debug: boolean}) {
  if (process.env.PWA_SW_CUSTOM) {
    ...
    if (typeof customSW.default === 'function') {
      ...
      console.warn(
        '[Ianaio-PWA][SW]: swCustom should have a default export function',
      );
    }
  }
}
...
(async () => {
  ...
  if (params.offlineMode) {
    ...
    if (params.debug) {
      console.log('[Ianaio-PWA][SW]: addToCacheList', {precacheManifest});
    }
  }
  ...
  self.addEventListener('install', (event) => {
    if (params.debug) {
      console.log('[Ianaio-PWA][SW]: install event', {event});
    }
  ...
  self.addEventListener('activate', (event) => {
    if (params.debug) {
      console.log('[Ianaio-PWA][SW]: activate event', {event});
    }
    (event as ExtendableEvent).waitUntil(
    ...
    if (params.offlineMode) {
      ...
      for (const possibleURL of possibleURLs) {
        ...
        if (cacheKey) {
          ...
          if (params.debug) {
            console.log('[Ianaio-PWA][SW]: serving cached asset', {
              requestURL,
              possibleURL,
              possibleURLs,
              cacheKey,
  ...
  self.addEventListener('message', async (event) => {
    if (params.debug) {
      console.log('[Ianaio-PWA][SW]: message event', {event});
    }
