...
export function parseLines(
  ...
  if (metastring && metastringLinesRangeRegex.test(metastring)) {
    ...
    if (magicComments.length === 0) {
      throw new Error(
        `A highlight range has been given in code block's metastring (\`\`\` ${metastring}), but no magic comment config is available. Ianaio applies the first magic comment entry's className for metastring ranges.`,
      );
    }
