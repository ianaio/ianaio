...
try {
  ...
  logger.info`This is non-critical, but could lead to undesired behavior downstream. Ianaio assumes that path=${'package.json'} exists at CWD, because it's where the package manager looks up the script at. A common reason is because you have changed directory in the script. Instead of writing code=${'"start": "cd website && ianaio start"'}, consider using the code=${'[siteDir]'} argument: code=${'"start": "ianaio start website"'}.`;
  sitePkg = {};
}
...
/**
 * Notify user if `@ianaio` packages are outdated
 *
 ...
 *
 * cache data is stored in `~/.config/configstore/update-notifier-@ianaio`
 */
export default async function beforeCli() {
  const notifier = updateNotifier({
    pkg: {
      name,
      version: IANAIO-VERSION,
    },
  ...
  try {
    ...
    // Do not stop cli if this fails, see https://github.com/ianaio/ianaio/issues/5400
    logger.error(err);
  }

  /**
   * We don't want to display update message for canary releases.
   * See https://github.com/ianaio/ianaio/issues/5378
  ...
  if (
    ...
    const siteIanaioPackagesForUpdate = Object.keys({
      ...sitePkg.dependencies,
      ...sitePkg.devDependencies,
    })
      .filter((p) => p.startsWith('@ianaio'))
    ...
    const getUpgradeCommand = async () => {
      ...
      if (!yarnVersion) {
        return `npm i ${siteIanaioPackagesForUpdate}`;
      }
      return yarnVersion >= 2
        ? `yarn up ${siteIanaioPackagesForUpdate}`
        : `yarn upgrade ${siteIanaioPackagesForUpdate}`;
    };
    ...
    const ianaioUpdateMessage = boxen(
      `Update available ${logger.dim(
        `${notifier.update.current}`,
      )} → ${logger.green(`${notifier.update.latest}`)}

  To upgrade Ianaio packages with the latest version, run the following command:
  ${logger.code(await getUpgradeCommand())}`,
      boxenOptions,
    );

    console.log(ianaioUpdateMessage);
  }
