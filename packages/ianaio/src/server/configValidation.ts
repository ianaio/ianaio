...
import {Joi, printWarning} from '@ianaio/utils-validation';
import type {IanaioConfig, I18nConfig} from '@ianaio/types';
...
> = {
  ...
  markdown: {
    format: 'mdx', // TODO change this to "detect" in Ianaio v4?
    mermaid: false,
...
export const ConfigSchema = Joi.object<IanaioConfig>({
  baseUrl: Joi
  ...
  scripts: Joi.array()
    .items(
      ...
      Joi.object({
        ...
        // See https://github.com/ianaio/ianaio/issues/3378
        .unknown(),
    )
    .messages({
      'array.includes':
  ...
  'ianaio.configValidationWarning':
    'Ianaio config validation warning. Field {#label}: {#warningMessage}',
});
