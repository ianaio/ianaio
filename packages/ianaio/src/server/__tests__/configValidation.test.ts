...
import type {Config, IanaioConfig} from '@ianaio/types';
...
const normalizeConfig = (config: DeepPartial<Config>) =>
  validateConfig({...baseConfig, ...config}, 'ianaio.config.js');
      ...
      scripts: [
        {
          ...
          async: true,
          defer: true,
          'data-domain': 'xyz', // See https://github.com/ianaio/ianaio/issues/3378
        },
      ],
      stylesheets: [
        {
...
describe('markdown', () => {
  ...
  it('accepts valid markdown object', () => {
    const markdown: IanaioConfig['markdown'] = {
      format: 'md',
      mermaid: true,
      preprocessor: ({fileContent}) => fileContent,
      mdx1Compat: {
  ...
  it('accepts partial markdown object', () => {
    const markdown: DeepPartial<IanaioConfig['markdown']> = {
      mdx1Compat: {
        admonitions: true,
        headingIds: false,
      },
