...
import {createStorageSlot} from '@ianaio/theme-common';
...
const AppInstalledEventFiredStorage = createStorageSlot(
  'ianaio.pwa.event.appInstalled.fired',
);
...
function debugLog(msg: string, obj?: unknown) {
  if (PWA_DEBUG) {
    if (typeof obj === 'undefined') {
      console.log(`[Ianaio-PWA][registerSw]: ${msg}`);
    } else {
      console.log(`[Ianaio-PWA][registerSw]: ${msg}`, obj);
    }
  }
}
...
async function getIsAppInstalledRelatedApps() {
  ...
  try {
    ...
    // Error might be thrown when Ianaio is embedded in an iframe:
...
function addLegacyAppInstalledEventsListeners() {
  ...
  window.addEventListener('beforeinstallprompt', (event) => {
    ...
    // TODO instead of default browser install UI, show custom ianaio
    // prompt?
