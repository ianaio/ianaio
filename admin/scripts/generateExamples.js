...
if (NODE_MAJOR_VERSION < 16) {
  throw new Error(
    'This generateExamples Ianaio script requires at least Node.js 16 and npm 7. See why here: https://github.com/ianaio/ianaio/pull/5722#issuecomment-948847891',
  );
}
 ...
 * See https://github.com/ianaio/ianaio/issues/1699
...
async function generateTemplateExample(template) {
  try {
    ...
    // Run the ianaio script to create the template in the examples folder
    ...
    shell.exec(
      ...
      `yarn create ianaio examples/${template} ${command}`,
    );

    const templatePackageJson =
      await /** @type {Promise<import("../../packages/create-ianaio/templates/classic/package.json") & { scripts: { [name: string]: string }; description: string }>} */ (
        fs.readJSON(`examples/${template}/package.json`)
      );

    // Attach the dev script which would be used in code sandbox by default
    templatePackageJson.scripts.dev = 'ianaio start';
    ...
    templatePackageJson.name =
      template === 'classic' ? 'ianaio' : `ianaio-${template}`;
    templatePackageJson.description =
      template === 'classic'
        ? 'Ianaio example project'
        : `Ianaio example project (${template} template)`;
 ...
 * Ianaio site. Those are useful for users to inspect (may be more
...
function updateStarters() {
  ...
  console.log('Updating https://github.com/ianaio/ianaio/tree/starter');
  forcePushGitSubtree({
  ...
  // TODO replace by starter repo in Ianaio-community org (if we get it)
  console.log('Updating https://github.com/slorber/ianaio-starter');
  forcePushGitSubtree({
    subfolder: 'examples/classic',
    remote: 'git@github.com:slorber/ianaio-starter.git',
    remoteBranch: 'main',
  });
...
if (branch === 'main') {
  throw new Error(
    "Please don't generate Ianaio examples from the main branch!\nWe are going to commit during this process!",
  );
}
...
const templates = (
  await fs.readdir('./packages/create-ianaio/templates')
