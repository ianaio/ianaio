...
We are happy to announce **Ianaio 2.1**, our very first minor version release.
...
![Ianaio 2.1 social card](./img/social-card.png)
...
In [#8008](https://github.com/ianaio/ianaio/pull/8008), we simplified the usage of the`<DocCardList>` component, that is notably used on sidebar category generated index pages.
...
- import {useCurrentSidebarCategory} from '@ianaio/theme-common';
...
In [#7963](https://github.com/ianaio/ianaio/pull/7963), we allow `noindex` to be configured on a per-docs-version basis.
...
In practice, Ianaio will add the following meta to each page of that version:

```html
<meta name="robots" content="noindex, nofollow" />
```

In [#7964](https://github.com/ianaio/ianaio/pull/7964), we also fixed a bug where the sitemap plugin would still contain pages that have a `noindex` directive. Now the sitemap plugin will reliably filter out all the pages containing `noindex` directives.

### Overriding default meta tags

In [#7952](https://github.com/ianaio/ianaio/pull/7952), it becomes possible to override default html meta tags you couldn't before:
...
In [#7953](https://github.com/ianaio/ianaio/pull/7953), we added default classic theme translations for the Ukrainian language.
