...
`context` is plugin-agnostic, and the same object will be passed into all plugins used for a Ianaio website. The `context` object contains the following fields:
...
// `context` is provided by Ianaio. Example: siteConfig can be accessed from context.
...
async function myPlugin(context, opts) {
  return {
    ...
    name: 'ianaio-my-project-cool-plugin',
    ...
    async contentLoaded({content, actions}) {
      ...
      // `actions` are set of functional API provided by Ianaio (e.g. addRoute)
    },

    async postBuild(props) {
      // After ianaio <build> finish.
    },
    ...
    extendCli(cli) {
      // Register an extra command to enhance the CLI of Ianaio
    },
