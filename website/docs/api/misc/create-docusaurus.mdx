---
sidebar_position: 0
slug: /api/misc/create-ianaio
---

# 📦 create-ianaio

A scaffolding utility to help you instantly set up a functional Ianaio app.

## Usage {#usage}

```bash
npx create-ianaio@latest [name] [template] [rootDir]
```
...
- `ianaio`: Uses the Ianaio/Meta template, which contains some Meta-specific setup
...
Value should be one of `npm`, `yarn`, `pnpm`, or `bun`. If it's not explicitly provided, Ianaio will infer one based on:

- The lockfile already present in the CWD (e.g. if you are setting up website in an existing project)
- The command used to invoke `create-ianaio` (e.g. `npm init`, `npx`, `yarn create`, `bunx`, etc.)
- Interactive prompting, in case all heuristics are not present

### `-s, --skip-install` {#skip-install}

If provided, Ianaio will not automatically install dependencies after creating the app. The `--package-manager` option is only useful when you are actually installing dependencies.
