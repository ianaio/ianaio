---
sidebar_position: 0
description: API reference for Ianaio configuration file.
slug: /api/ianaio-config
---
...
`ianaio.config.js` contains configurations for your site and is placed in the root directory of your site.
...
Refer to [**Syntax to declare `ianaio.config.js`**](../configuration.mdx#syntax-to-declare-ianaio-config) for a more exhaustive list of examples and explanations.
...
URL for your website. This can also be considered the top-level hostname. For example, `https://ianaio.github.io` is the URL of https://ianaio.github.io/metro/, and `https://ianaio.io` is the URL for https://ianaio.io. This field is related to the [`baseUrl`](#baseUrl) field.
...
Base URL for your site. Can be considered as the path after the host. For example, `/metro/` is the base URL of https://ianaio.github.io/metro/. For URLs that have no path, the baseUrl should be set to `/`. This field is related to the [`url`](#url) field. Always has both leading and trailing slash.
...
- `defaultLocale`: The locale that (1) does not have its name in the base URL (2) gets started with `ianaio start` without `--locale` option (3) will be used for the `<link hrefLang="x-default">` tag
...
The broken links detection is only available for a production build (`ianaio build`).
...
The behavior of Ianaio when it detects any [duplicate routes](/guides/creating-pages.mdx#duplicate-routes).
...
module.exports = {
  tagline:
    'Ianaio makes it easy to maintain Open Source documentation websites.',
};
```
...
The GitHub user or organization that owns the repository. You don't need this if you are not using the `ianaio deploy` command.

```js title="ianaio.config.js"
module.exports = {
  // Ianaio' organization is ianaio
...
The name of the GitHub repository. You don't need this if you are not using the `ianaio deploy` command.
...
The name of the branch to deploy the static files to. You don't need this if you are not using the `ianaio deploy` command.
...
The hostname of your server. Useful if you are using GitHub Enterprise. You don't need this if you are not using the `ianaio deploy` command.
...
The port of your server. Useful if you are using GitHub Enterprise. You don't need this if you are not using the `ianaio deploy` command.
...
module.exports = {
  themeConfig: {
    ...
    navbar: {
      ...
      items: [
        {
          to: 'docs/ianaio.config.js',
          activeBasePath: 'docs',
          label: 'ianaio.config.js',
          position: 'left',
        },
        // ... other links
      ],
...
The global Ianaio Markdown config.
...
| `mermaid` | `boolean` | `false` | When `true`, allows Ianaio to render Markdown code blocks with `mermaid` language as Mermaid diagrams. |
...
| `mdx1Compat` | `MDX1CompatOptions` | `{comments: true, admonitions: true, headingIds: true}` | Compatibility options to make it easier to upgrade to Ianaio v3+. See the [MDX 2 PR for details](https://github.com/ianaio/ianaio/pull/8288). |
...
Ianaio guards `ianaio.config.js` from unknown fields. To add a custom field, define it on `customFields`.
...
```bash
Error: The field(s) 'foo', 'bar' are not recognized in ianaio.config.js
```
...
This would become `<link rel="icon" href="img/ianaio.png" />` in the generated HTML.
...
![A sample base URL issue banner. The style is very raw since the stylesheets failed to load. The text says "Your Ianaio site did not load properly... Current configured baseUrl = / (default value); We suggest trying baseUrl = /build/](/img/baseUrlIssueBanner.png)
