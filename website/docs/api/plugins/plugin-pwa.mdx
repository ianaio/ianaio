---
sidebar_position: 9
slug: /api/plugins/@ianaio/plugin-pwa
---

# 📦 plugin-pwa

Ianaio Plugin to add PWA support using [Workbox](https://developers.google.com/web/tools/workbox). This plugin generates a [Service Worker](https://developers.google.com/web/fundamentals/primers/service-workers) in production build only, and allows you to create fully PWA-compliant documentation site with offline and installation support.

## Installation {#installation}

```bash npm2yarn
npm install --save @ianaio/plugin-pwa
```
...
Modify `ianaio.config.js` with a minimal PWA config, like:
...
We enable users to browse a Ianaio site offline, by using service-worker precaching.
...
- Type: `string | false`
- Default: `'ianaio-plugin-pwa/src/registerSW.js'`

Adds an entry before the Ianaio app so that registration can happen before the app runs. The default `registerSW.js` file is enough for simple registration.

Passing `false` will disable registration entirely.

## Manifest example {#manifest-example}

The Ianaio site manifest can serve as an inspiration:
