...
Ianaio provides some APIs on the clients that can be helpful to you when building your site.
...
Ianaio uses this component to catch errors within the theme's layout, and also within the entire app.
...
The component is a wrapper around react-router’s `<Link>` component that adds useful enhancements specific to Ianaio. All props are passed through to react-router’s `<Link>` component.
...
const Page = () => (
  <div>
    ...
    <p>
      {/* highlight-next-line */}
      Follow me on <Link to="https://twitter.com/ianaio">Twitter</Link>!
    </p>
  </div>
);
```
...
Prefer this component to vanilla `<a>` tags because Ianaio does a lot of optimizations (e.g. broken path detection, prefetching, applying base URL...) if you use `<Link>`.
...
`@ianaio/router` implements [React Router](https://reacttraining.com/react-router/web/guides/quick-start) and supports its features.
  ...
  return (
    ...
    <Interpolate
      values={{
        firstName: 'Sébastien',
        website: (
          <Link to="https://ianaio.io" className="my-website-class">
            website
          </Link>
        ),
      }}>
...
The translation strings will statically extracted from your code with the [`ianaio write-translations`](./cli.mdx#ianaio-write-translations-sitedir) CLI and a `code.json` translation file will be created in `website/i18n/[locale]`.
...
You can even omit the children prop and specify a translation string in your `code.json` file manually after running the `ianaio write-translations` CLI command.
...
The `<Translate>` component supports interpolation. You can also implement [string pluralization](https://github.com/ianaio/ianaio/pull/i18n/i18n-tutorial.mdx#pluralization) through some custom code and the [`translate` imperative API](#translate-imperative).

:::

## Hooks {#hooks}

### `useIanaioContext` {#useIanaioContext}

React hook to access Ianaio Context. The context contains the `siteConfig` object from [ianaio.config.js](api/ianaio.config.js.mdx) and some additional site metadata.
...
const MyComponent = () => {
  // highlight-next-line
  const {siteConfig, siteMetadata} = useIanaioContext();
  return (
    <div>
      {/* highlight-start */}
      <h1>{siteConfig.title}</h1>
      <div>{siteMetadata.siteVersion}</div>
      <div>{siteMetadata.ianaioVersion}</div>
      {/* highlight-end */}
    </div>
  );
};
...
React hook to access Ianaio global data created by all the plugins.
...
Inspect your site's global data at `./ianaio/globalData.json`
...
import React from 'react';
// highlight-next-line
import {usePluginData} from '@ianaio/useGlobalData';
...
import React from 'react';
// highlight-next-line
import {useAllPluginInstancesData} from '@ianaio/useGlobalData';
