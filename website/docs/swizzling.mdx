...
In this section, we will introduce how customization of layout is done in Ianaio.

> Déja vu...?

This section is similar to [Styling and Layout](./styling-layout.mdx), but this time, we will customize React components themselves, rather than what they look like. We will talk about a central concept in Ianaio: **swizzling**, which allows **deeper site customizations**.
...
**For Ianaio, component swizzling means providing an alternative component that takes precedence over the component provided by the theme.**
...
Ianaio provides a convenient **interactive CLI** to swizzle components. You generally only need to remember the following command:
...
Use `--help` to see all available CLI options, or refer to the reference [swizzle CLI documentation](./cli.mdx#ianaio-swizzle).

:::note

After swizzling a component, **restart your dev server** in order for Ianaio to know about the new component.
...
`ianaio swizzle` is only an automated way to help you swizzle the component. You can also create the `src/theme/SomeComponent.js` file manually, and Ianaio will [resolve it](./advanced/client.mdx#theme-aliases). There's no internal magic behind this command!
...
```bash npm2yarn
npm run swizzle @ianaio/theme-classic Footer -- --eject
```

This will copy the current `<Footer />` component's implementation to your site's `src/theme` directory. Ianaio will now use this `<Footer>` component copy instead of the original one. You are now free to completely re-implement the `<Footer>` component.
...
Ejecting an [**unsafe**](#what-is-safe-to-swizzle) component can sometimes lead to copying a large amount of internal code, which you now have to maintain yourself. It can make Ianaio upgrades more difficult, as you will need to migrate your customizations if the props received or internal theme APIs used have changed.
...
To keep ejected components up-to-date after a Ianaio upgrade, re-run the eject command and compare the changes with `git diff`. You are also recommended to write a brief comment at the top of the file explaining what changes you have made, so that you could more easily re-apply your changes after re-ejection.
...
```bash npm2yarn
npm run swizzle @ianaio/theme-classic Footer -- --wrap
```

This will create a wrapper in your site's `src/theme` directory. Ianaio will now use the `<FooterWrapper>` component instead of the original one. You can now add customizations around the original component.
...
Some theme components are **internal implementation details** of a theme. Ianaio allows you to swizzle them, but it **might be risky**.
...
**Theme components marked as unsafe may change in a backward-incompatible way between theme minor versions.** When upgrading a theme (or Ianaio), your customizations might **behave unexpectedly**, and can even **break your site**.
...
If you have a **strong use-case for swizzling an unsafe component**, please [**report it here**](https://github.com/ianaio/ianaio/discussions/5468) and we will work together to find a solution to make it safe.

:::

## Which component should I swizzle? {#which-component-should-i-swizzle}

It is not always clear which component you should swizzle exactly to achieve the desired result. `@ianaio/theme-classic`, which provides most of the theme components, has about [100 components](https://github.com/ianaio/ianaio/tree/main/packages/ianaio-theme-classic/src/theme)!

:::tip

To print an overview of all the `@ianaio/theme-classic` components:

```bash npm2yarn
npm run swizzle @ianaio/theme-classic -- --list
```
...
2. **Component name.** Official theme components are semantically named, so you should be able to infer its function from the name. The swizzle CLI allows you to enter part of a component name to narrow down the available choices. For example, if you run `yarn swizzle @ianaio/theme-classic`, and enter `Doc`, only the docs-related components will be listed.
...
4. **Read the [theme source code](https://github.com/ianaio/ianaio/tree/main/packages/ianaio-theme-classic/src/theme)** and use search wisely.
...
We also want to understand better your fanciest customization use-cases, so please [**report them**](https://github.com/ianaio/ianaio/discussions/5468).

:::

## Do I need to swizzle? {#do-i-need-to-swizzle}

Swizzling ultimately means you have to maintain some additional React code that interact with Ianaio internal APIs. If you can, think about the following alternatives when customizing your site:
