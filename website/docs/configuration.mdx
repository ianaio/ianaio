...
Check the [**`ianaio.config.js` API reference**](api/ianaio.config.js.mdx) for an exhaustive list of options.
...
## Syntax to declare `ianaio.config.js` {#syntax-to-declare-ianaio-config}

The `ianaio.config.js` file is run in Node.js and should export either:
...
The `ianaio.config.js` file only supports the [**CommonJS**](https://flaviocopes.com/commonjs/) module system:

- **Required:** use `module.exports = /* your config*/` to export your Ianaio config
...
Node.js gives us the ability to declare our Ianaio configuration in various **equivalent ways**, and all the following config examples lead to the exact same result:
