...
Plugins are the building blocks of features in a Ianaio 2 site. Each plugin handles its own individual feature. Plugins may work and be distributed as part of a bundle via presets.
...
You can use a plugin as a function directly included in the Ianaio config file:
...
You can view all plugins installed in your site using the [debug plugin's metadata panel](/__ianaio/debug/metadata).

Plugins come as several types:

- `package`: an external package you installed
- `project`: a plugin you created in your project, given to Ianaio as a local file path
- `local`: a plugin created using the function definition
- `synthetic`: a "fake plugin" Ianaio created internally, so we take advantage of our modular architecture and don't let the core do much special work. You won't see this in the metadata because it's an implementation detail.

You can access them on the client side with `useIanaioContext().siteMetadata.pluginVersions`.

## Plugin design {#plugin-design}

Ianaio' implementation of the plugins system provides us with a convenient way to hook into the website's lifecycle to modify what goes on during development/build, which involves (but is not limited to) extending the webpack config, modifying the data loaded, and creating new components to be used in a page.
...
For example, a Ianaio blog may consist of a blog plugin and a blog theme.

:::note

This is a contrived example: in practice, `@ianaio/theme-classic` provides the theme for docs, blog, and layouts.
...
Themes are designed to complete the build of your Ianaio site and supply the components used by your site, plugins, and the themes themselves. A theme still acts like a plugin and exposes some lifecycle methods, but most likely they would not use [`loadContent`](../api/plugin-methods/lifecycle-apis.mdx#loadContent), since they only receive data from plugins, but don't generate data themselves; themes are typically also accompanied by an `src/theme` directory full of components, which are made known to the core through the [`getThemePath`](../api/plugin-methods/extend-infrastructure.mdx#getThemePath) lifecycle.
