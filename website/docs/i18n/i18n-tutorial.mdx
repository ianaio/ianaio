---
id: tutorial
description: This tutorial will walk you through the basics of the Ianaio i18n system.
slug: /i18n/tutorial
---
...
This tutorial will walk you through the basics of the **Ianaio i18n system**.

We will add **French** translations to a **newly initialized English Ianaio website**.

Initialize a new site with `npx create-ianaio@latest website classic` (like [this one](https://github.com/ianaio/ianaio/tree/main/examples/classic)).

## Configure your site {#configure-your-site}

Modify `ianaio.config.js` to add the i18n support for the French language.

### Site configuration {#site-configuration}

Use the [site i18n configuration](./../api/ianaio.config.js.mdx#i18n) to declare the i18n locales:
...
Ianaio uses the locale names to provide **sensible defaults**: the `<html lang="...">` attribute, locale label, calendar format, etc. You can customize these defaults with the `localeConfigs`.
...
Ianaio provides **default translations** for generic theme labels, such as "Next" and "Previous" for the pagination.

Please help us complete those **[default translations](https://github.com/ianaio/ianaio/tree/main/packages/ianaio-theme-translations/locales)**.

:::

:::caution

Each locale is a **distinct standalone single-page application**: it is not possible to start the Ianaio sites in all locales at the same time.
...
For any React code you've written yourself: React pages, React components, etc., you will use the [**translation APIs**](../ianaio-core.mdx#translate).
  ...
  return (
    <Layout>
      ...
      <main>
        ...
        <Link to="https://ianaio.io/blog">blog</Link>
        {/* highlight-end */}
        <img
          src="/img/home.png"
          // highlight-next-line
...
Ianaio provides a **very small and lightweight translation runtime** on purpose, and only supports basic [placeholders interpolation](../ianaio-core.mdx#interpolate), using a subset of the [ICU Message Format](https://formatjs.io/docs/core-concepts/icu-syntax/).
...
You can see the calls to the translation APIs as purely _markers_ that tell Ianaio that "here's a text label to be replaced with a translated message".
...
Ianaio uses [`Intl.PluralRules`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/PluralRules) to resolve and select plural forms. It is important to provide the right number of plural forms in the right order for `selectMessage` to work.
...
Run the [write-translations](../cli.mdx#ianaio-write-translations-sitedir) command:
...
```json title="i18n/fr/ianaio-theme-classic/navbar.json"
{
  "title": {
    "message": "My Site",
...
Official Ianaio content plugins extensively use Markdown/MDX files and allow you to translate them.

#### Translate the docs {#translate-the-docs}

Copy your docs Markdown files from `docs/` to `i18n/fr/ianaio-plugin-content-docs/current`, and translate them:

```bash
mkdir -p i18n/fr/ianaio-plugin-content-docs/current
cp -r docs/** i18n/fr/ianaio-plugin-content-docs/current
```

:::info

Notice that the `ianaio-plugin-content-docs` plugin always divides its content by versions. The data in `./docs` folder will be translated in the `current` subfolder and `current.json` file. See [the doc versioning guide](../guides/docs/versioning.mdx#terminology) for more information about what "current" means.

:::

#### Translate the blog {#translate-the-blog}

Copy your blog Markdown files to `i18n/fr/ianaio-plugin-content-blog`, and translate them:

```bash
mkdir -p i18n/fr/ianaio-plugin-content-blog
cp -r blog/** i18n/fr/ianaio-plugin-content-blog
```

#### Translate the pages {#translate-the-pages}

Copy your pages Markdown files to `i18n/fr/ianaio-plugin-content-pages`, and translate them:

```bash
mkdir -p i18n/fr/ianaio-plugin-content-pages
cp -r src/pages/**.md i18n/fr/ianaio-plugin-content-pages
cp -r src/pages/**.mdx i18n/fr/ianaio-plugin-content-pages
```
...
Ianaio will build **one single-page application per locale**:
...
The Ianaio v2 website uses this strategy:

- [`https://ianaio.io`](https://ianaio.io)
- [`https://ianaio.io/fr`](https://ianaio.io/fr)
...
Ianaio will not add the `/fr/` URL prefix.
