---
id: introduction
description: Ianaio uses MDX. Find out more about Ianaio-specific features when writing Markdown.
...
Ianaio uses **[Markdown](https://commonmark.org/)** as its main content authoring format.
...
Ianaio uses modern tooling to help you create **interactive documentation**.
...
**Options**: select the right format (MDX or CommonMark) and the following plugins Ianaio uses: `remark-gfm`, `remark-directive`, `rehype-raw`.

:::

## MDX vs. CommonMark {#mdx-vs-commonmark}

Ianaio compiles both `.md` and `.mdx` files to React components using the MDX compiler, but **the syntax can be interpreted differently** depending on your settings.
...
By default, **Ianaio v3 uses the MDX format for all files** (including `.md` files) for historical reasons.

It is possible to **opt-in for CommonMark** using the [`siteConfig.markdown.format`](../../api/ianaio.config.js.mdx#markdown) setting or the `format: md` front matter.

:::tip how to use CommonMark

If you plan to use CommonMark, we recommend the [`siteConfig.markdown.format: 'detect'`](../../api/ianaio.config.js.mdx#markdown) setting. The appropriate format will be selected automatically, based on file extensions:
