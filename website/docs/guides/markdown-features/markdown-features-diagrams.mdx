...
Enable Mermaid functionality by adding plugin `@ianaio/theme-mermaid` and setting `markdown.mermaid` to `true` in your `ianaio.config.js`.
...
The diagram dark and light themes can be changed by setting `mermaid.theme` values in the `themeConfig` in your `ianaio.config.js`. You can set themes for both light and dark mode.
