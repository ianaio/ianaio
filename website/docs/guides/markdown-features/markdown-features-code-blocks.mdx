---
id: code-blocks
description: Handling code blocks in Ianaio Markdown
...
Use the matching language meta string for your code block, and Ianaio will pick up syntax highlighting automatically, powered by [Prism React Renderer](https://github.com/FormidableLabs/prism-react-renderer).
...
Because a Prism theme is just a JS object, you can also write your own theme if you are not satisfied with the default. Ianaio enhances the `github` and `vsDark` themes to provide richer highlight, and you can check our implementations for the [light](https://github.com/ianaio/ianaio/blob/main/website/src/utils/prismLight.mjs) and [dark](https://github.com/ianaio/ianaio/blob/main/website/src/utils/prismDark.mjs) code block themes.

### Supported Languages {#supported-languages}

By default, Ianaio comes with a subset of [commonly used languages](https://github.com/FormidableLabs/prism-react-renderer/blob/master/packages/generate-prism-languages/index.ts#L9-L23).
...
After adding `additionalLanguages`, restart Ianaio.

If you want to add highlighting for languages not yet supported by Prism, you can swizzle `prism-include-languages`:

```bash npm2yarn
npm run swizzle @ianaio/theme-classic prism-include-languages
```
...
When adding a custom language definition, you do not need to add the language to the `additionalLanguages` config array, since Ianaio only looks up the `additionalLanguages` strings in languages that Prism provides. Adding the language import in `prism-include-languages.js` is sufficient.
...
```mdx-code-block
<Tabs>
<TabItem value="ianaio.config.js">
```
...
If you use number ranges in metastring (the `{1,3-4}` syntax), Ianaio will apply the **first `magicComments` entry**'s class name. This, by default, is `theme-code-block-highlighted-line`, but if you change the `magicComments` config and use a different entry as the first one, the meaning of the metastring range will change as well.

You can disable the default line highlighting comments with `magicComments: []`. If there's no magic comment config, but Ianaio encounters a code block containing a metastring range, it will error because there will be no class name to apply—the highlighting class name, after all, is just a magic comment entry.
...
```bash npm2yarn
npm run swizzle @ianaio/theme-classic CodeBlock/Line
```
...
You can create an interactive coding editor with the `@ianaio/theme-live-codeblock` plugin. First, add the plugin to your package.

```bash npm2yarn
npm install --save @ianaio/theme-live-codeblock
```

You will also need to add the plugin to your `ianaio.config.js`.
...
```bash npm2yarn
npm run swizzle @ianaio/theme-live-codeblock ReactLiveScope -- --eject
```
...
Syntax highlighting only works on plain strings. Ianaio will not attempt to parse code block content containing JSX children.
...
### Ianaio npm2yarn remark plugin {#npm2yarn-remark-plugin}
...
Ianaio provides such a utility out of the box, freeing you from using the `Tabs` component every time. To enable this feature, first install the `@ianaio/remark-plugin-npm2yarn` package as above, and then in `ianaio.config.js`, for the plugins where you need this feature (doc, blog, pages, etc.), register it in the `remarkPlugins` option. (See [Docs configuration](../../api/plugins/plugin-content-docs.mdx#ex-config) for more details on configuration format)
  ...
  presets: [
    [
      '@ianaio/preset-classic',
      {
        ...
        pages: {
          // highlight-next-line
          remarkPlugins: [require('@ianaio/remark-plugin-npm2yarn')],
        },
        blog: {
          // highlight-start
          remarkPlugins: [
