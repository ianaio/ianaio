...
To avoid mixing Joi versions, use `const {Joi} = require("@ianaio/utils-validation")`
...
`validateThemeConfig` is called with `themeConfig` provided in `ianaio.config.js` for validation and normalization.
