...
A freshly initialized Ianaio site has the following structure:
...
There's also a "blog-only mode" for those who only want to use the blog feature of Ianaio 2. You can use the same method detailed above. Follow the setup instructions on [Blog-only mode](../../blog.mdx#blog-only-mode).
