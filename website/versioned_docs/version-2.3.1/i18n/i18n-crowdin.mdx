...
The i18n system of Ianaio is **decoupled from any translation software**.

You can integrate Ianaio with the **tools and SaaS of your choice**, as long as you put the **translation files at the correct location**.

We document the usage of [Crowdin](https://crowdin.com/), as **one** possible **integration example**.

:::caution

This is **not an endorsement of Crowdin** as the unique choice to translate a Ianaio site, but it is successfully used by Ianaio to translate documentation projects such as [Jest](https://jestjs.io/), [Ianaio](https://ianaio.io/), and [ReasonML](https://reasonml.github.io/).
...
Use this **[community-driven GitHub discussion](https://github.com/ianaio/ianaio/discussions/4052)** to discuss anything related to Ianaio + Crowdin.
...
The [`crowdin.yml` configuration file](https://support.crowdin.com/configuration-file/) is convenient for Ianaio, and permits to **download the localized translation files at the expected location** (in `i18n/[locale]/..`).
...
This is a walk-through of using Crowdin to translate a newly initialized English Ianaio website into French, and assume you already followed the [i18n tutorial](./i18n-tutorial.mdx).

The end result can be seen at [ianaio-crowdin-example.netlify.app](https://ianaio-crowdin-example.netlify.app/) ([repository](https://github.com/slorber/ianaio-crowdin-example)).

### Prepare the Ianaio site {#prepare-the-ianaio-site}

Initialize a new Ianaio site:

```bash
npx create-ianaio@latest website classic
```
    ...
    <Layout>
      <h1 style={{margin: 20}}>
        <Translate description="The homepage main heading">
          Welcome to my Ianaio translated site!
        </Translate>
      </h1>
    </Layout>
  );
...
files:
  ...
  # Docs Markdown files
  - source: /docs/**/*
    translation: /i18n/%two_letters_code%/ianaio-plugin-content-docs/current/**/%original_file_name%
  # Blog Markdown files
  - source: /blog/**/*
    translation: /i18n/%two_letters_code%/ianaio-plugin-content-blog/**/%original_file_name%
```
...
Install the Crowdin CLI as an npm package to your Ianaio site:
...
```json title="package.json"
{
  "scripts": {
    // ...
    "write-translations": "ianaio write-translations",
    "crowdin": "crowdin"
  }
}
```
...
![Crowdin CLI uploading Ianaio source files](/img/crowdin/crowdin-upload-sources-cli.png)

Your source files are now visible on the Crowdin interface: `https://crowdin.com/project/<MY_PROJECT_NAME>/settings#files`

![Crowdin UI showing Ianaio source files](/img/crowdin/crowdin-source-files.png)
...
{
  "scripts": {
    "crowdin:sync": "ianaio write-translations && crowdin upload && crowdin download"
  }
}
```

Call the `npm run crowdin:sync` script in your CI, just before building the Ianaio site.
...
`````text
# How to deploy Ianaio

To deploy Ianaio, run the following command:
...
- be interpreted by Ianaio as regular JSX (as if it was not wrapped by any code block)
...
The **Ianaio v2 configuration file** is a good example of using versioning and multi-instance:
