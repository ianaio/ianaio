...
Then you add it in your site's `ianaio.config.js`'s `plugins` option:

```js title="ianaio.config.js"
module.exports = {
  // ...
  // highlight-next-line
  plugins: ['@ianaio/plugin-content-pages'],
};
```

Ianaio can also load plugins from your local directory, with something like the following:

```js title="ianaio.config.js"
module.exports = {
  // ...
  // highlight-next-line
  plugins: ['./src/plugins/ianaio-local-plugin'],
};
```
...
module.exports = {
  // ...
  // highlight-next-line
  themes: ['@ianaio/theme-classic', '@ianaio/theme-live-codeblock'],
};
```

## Using presets {#using-presets}

Presets are bundles of plugins and themes. For example, instead of letting you register and configure `@ianaio/plugin-content-docs`, `@ianaio/plugin-content-blog`, etc. one after the other in the config file, we have `@ianaio/preset-classic` preset allows you to configure them in one centralized place.

### `@ianaio/preset-classic` {#ianaiopreset-classic}

The classic preset is shipped by default to new Ianaio websites created with [`create-ianaio`](./installation.mdx#scaffold-project-website). It contains the following themes and plugins:

- [`@ianaio/theme-classic`](./api/themes/theme-configuration.mdx)
- [`@ianaio/theme-search-algolia`](./api/themes/theme-search-algolia.mdx)
- [`@ianaio/plugin-content-docs`](./api/plugins/plugin-content-docs.mdx)
- [`@ianaio/plugin-content-blog`](./api/plugins/plugin-content-blog.mdx)
- [`@ianaio/plugin-content-pages`](./api/plugins/plugin-content-pages.mdx)
- [`@ianaio/plugin-debug`](./api/plugins/plugin-debug.mdx)
- [`@ianaio/plugin-google-gtag`](./api/plugins/plugin-google-gtag.mdx)
- [`@ianaio/plugin-google-tag-manager`](./api/plugins/plugin-google-tag-manager.mdx)
- [`@ianaio/plugin-google-analytics`](./api/plugins/plugin-google-analytics.mdx) (**deprecated**)
- [`@ianaio/plugin-sitemap`](./api/plugins/plugin-sitemap.mdx)
  ...
  presets: [
    [
      '@ianaio/preset-classic',
      {
        ...
        debug: undefined,
        // Will be passed to @ianaio/theme-classic.
        theme: {
          customCss: [require.resolve('./src/css/custom.css')],
        },
        // Will be passed to @ianaio/plugin-content-docs (false to disable)
        docs: {},
        // Will be passed to @ianaio/plugin-content-blog (false to disable)
        blog: {},
        // Will be passed to @ianaio/plugin-content-pages (false to disable)
        pages: {},
        // Will be passed to @ianaio/plugin-content-sitemap (false to disable)
        sitemap: {},
        // Will be passed to @ianaio/plugin-google-gtag (only enabled when explicitly specified)
        gtag: {},
        // Will be passed to @ianaio/plugin-google-tag-manager (only enabled when explicitly specified)
        googleTagManager: {},
        // DEPRECATED: Will be passed to @ianaio/plugin-google-analytics (only enabled when explicitly specified)
        googleAnalytics: {},
      },
    ],
  ],
...
```bash npm2yarn
npm install --save @ianaio/preset-classic
```

Then, add it in your site's `ianaio.config.js`'s `presets` option:
...
module.exports = {
  // ...
  // highlight-next-line
  presets: ['./src/presets/ianaio-local-preset'],
};
```
...
```js title="src/presets/ianaio-preset-multi-docs.js"
module.exports = function preset(context, opts = {}) {
  return {
    themes: [['ianaio-theme-awesome', opts.theme]],
    plugins: [
      // Using three docs plugins at the same time!
      // Assigning a unique ID for each without asking the user to do it
      ['@ianaio/plugin-content-docs', {...opts.docs1, id: 'docs1'}],
      ['@ianaio/plugin-content-docs', {...opts.docs2, id: 'docs2'}],
      ['@ianaio/plugin-content-docs', {...opts.docs3, id: 'docs3'}],
    ],
  };
};
```

Then in your Ianaio config, you may configure the preset:
...
module.exports = {
  presets: [
    // highlight-start
    [
      './src/presets/ianaio-preset-multi-docs.js',
      {
        theme: {hello: 'world'},
        docs1: {path: '/docs'},
...
```js title="ianaio.config.js"
module.exports = {
  themes: [['ianaio-theme-awesome', {hello: 'world'}]],
  plugins: [
    ['@ianaio/plugin-content-docs', {id: 'docs1', path: '/docs'}],
    ['@ianaio/plugin-content-docs', {id: 'docs2', path: '/community'}],
    ['@ianaio/plugin-content-docs', {id: 'docs3', path: '/api'}],
  ],
};
```
...
Ianaio supports shorthands for plugins, themes, and presets. When it sees a plugin/theme/preset name, it tries to load one of the following, in that order:

- `[name]` (like `content-docs`)
- `@ianaio/[moduleType]-[name]` (like `@ianaio/plugin-content-docs`)
- `ianaio-[moduleType]-[name]` (like `ianaio-plugin-content-docs`)
...
If there is no name (like `@jquery`), `[scope]/ianaio-[moduleType]` (i.e. `@jquery/ianaio-plugin`) is loaded. Otherwise, the following are attempted:

- `[scope]/[name]` (like `@jquery/content-docs`)
- `[scope]/ianaio-[moduleType]-[name]` (like `@jquery/ianaio-plugin-content-docs`)

Below are some examples, for a plugin registered in the `plugins` field. Note that unlike [ESLint](https://eslint.org/docs/user-guide/configuring/plugins#configuring-plugins) or [Babel](https://babeljs.io/docs/en/options#name-normalization) where a consistent naming convention for plugins is mandated, Ianaio permits greater naming freedom, so the resolutions are not certain, but follows the priority defined above.

| Declaration | May be resolved as |
| --- | --- |
| `awesome` | `ianaio-plugin-awesome` |
| `sitemap` | [`@ianaio/plugin-sitemap`](./api/plugins/plugin-sitemap.mdx) |
| `@my-company` | `@my-company/ianaio-plugin` (the only possible resolution!) |
| `@my-company/awesome` | `@my-company/ianaio-plugin-awesome` |
| `@my-company/awesome/web` | `@my-company/ianaio-plugin-awesome/web` |
