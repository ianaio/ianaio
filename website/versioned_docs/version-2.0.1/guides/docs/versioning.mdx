...
```bash npm2yarn
npm run ianaio docs:version 1.1.0
```
...
Ianaio defaults work great for the first use case. We will label the current version as "next" and you can even choose not to publish it.
...
Refer to other docs by relative file paths with the `.md` extension, so that Ianaio can rewrite them to actual URL paths during building. Files will be linked to the correct corresponding version.
