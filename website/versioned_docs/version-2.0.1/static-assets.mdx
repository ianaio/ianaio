...
- for site `baseUrl: '/'`, the image `/static/img/ianaio.png` will be served at `/img/ianaio.png`.
- for site `baseUrl: '/subpath/'`, the image `/static/img/ianaio.png` will be served at `/subpath/img/ianaio.png`.

You can customize the static directory sources in `ianaio.config.js`. For example, we can add `public` as another possible path:
...
In JSX, you can reference assets from the `static` folder in your code using absolute URLs, but this is not ideal because changing the site `baseUrl` will **break those links**. For the image `<img src="/img/ianaio.png" />` served at `https://example.com/test`, the browser will try to resolve it from the URL root, i.e. as `https://example.com/img/ianaio.png`, which will fail because it's actually served at `https://example.com/test/img/ianaio.png`.
...
import IanaioImageUrl from '@site/static/img/ianaio.png';

<img src={IanaioImageUrl} />;
```
...
```jsx title="MyComponent.js"
import IanaioLogoWithKeytar from '@site/static/img/ianaio_keytar.svg';

<IanaioLogoWithKeytar title="Ianaio Logo" className="logo" />;
```

### In Markdown {#in-markdown}

In Markdown, you can stick to using absolute paths when writing links or images **in Markdown syntax** because Ianaio handles them as `require` calls instead of URLs when parsing the Markdown. See [Markdown static assets](./guides/markdown-features/markdown-features-assets.mdx).

```md
You write a link like this: [Download this document](/files/note.docx)

Ianaio changes that to: <a href={require('static/files/note.docx')}>Download this document</a>
```

:::caution use Markdown syntax

Ianaio will only parse links that are in Markdown syntax. If your asset references are using the JSX tag `<a>` / `<img>`, nothing will be done.
