...
```js title="ianaio-plugin/src/index.js"
...
```js title="ianaio-friends-plugin/src/index.js"
export default function friendsPlugin(context, options) {
  return {
    name: 'ianaio-friends-plugin',
    // highlight-start
...
This data becomes accessible to your client-side/theme code through the [`useGlobalData`](../../ianaio-core.mdx#useGlobalData) and [`usePluginData`](../../ianaio-core.mdx#usePluginData) hooks.
...
export default function FriendsComponent() {
  const {friends} = usePluginData('ianaio-friends-plugin');
  return <div>Your friends are {friends.join(',')}</div>;
}
```
...
Inject head and/or body HTML tags to Ianaio generated HTML.
