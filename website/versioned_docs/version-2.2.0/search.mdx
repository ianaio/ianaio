...
🥇 Ianaio provides **first-class support** for [Algolia DocSearch](#using-algolia-docsearch).
...
Ianaio has **official support** for [Algolia DocSearch](https://docsearch.algolia.com).
...
By default, the Ianaio preset generates a [sitemap.xml](https://ianaio.io/sitemap.xml) that the Algolia crawler can use.
...
It is highly recommended to use a config similar to the [**Ianaio 2 website config**](https://docsearch.algolia.com/docs/templates/#ianaio-v2-template).

:::

### Connecting Algolia {#connecting-algolia}

Ianaio' own `@ianaio/preset-classic` supports Algolia DocSearch integration. If you use the classic preset, no additional installation is needed.

<details>
<summary>Installation steps when not using <code>@ianaio/preset-classic</code></summary>
...
2. Register the theme in `ianaio.config.js`:

```js title="ianaio.config.js"
module.exports = {
  title: 'My site',
  // ...
  themes: ['@ianaio/theme-search-algolia'],
  themeConfig: {
    // ...
  },
};
...
The `searchParameters` option used to be named `algoliaOptions` in Ianaio v1.
...
Still, you can reuse the [Infima CSS variables](styling-layout.mdx#styling-your-site-with-infima) from Ianaio to style DocSearch by editing the `/src/css/custom.css` file.
...
Algolia DocSearch supports a [list of options](https://autocomplete-experimental.netlify.app/docs/DocSearchModal#reference) that you can pass to the `algolia` field in the `ianaio.config.js` file.
...
If you prefer to edit the Algolia search React component, [swizzle](swizzling.mdx) the `SearchBar` component in `@ianaio/theme-search-algolia`:
...
Ianaio also has an `#algolia` channel on [Discord](https://discordapp.com/invite/ianaio).
...
- [ianaio-theme-search-typesense](https://github.com/typesense/ianaio-theme-search-typesense) - a search bar UI component to add to your website.

Read a step-by-step walk-through of how to [run typesense-docsearch-scraper here](https://typesense.org/docs/latest/guide/docsearch.html#step-1-set-up-docsearch-scraper) and how to [install the Search Bar in your Ianaio Site here](https://typesense.org/docs/latest/guide/docsearch.html#option-a-ianaio-powered-sites).
...
You'll find a list of community-supported [local search plugins listed here](https://ianaio.io/community/resources#search).

## 👥 Using your own search {#using-your-own-search}

To use your own search, swizzle the `SearchBar` component in `@ianaio/theme-classic`

```bash npm2yarn
npm run swizzle @ianaio/theme-classic SearchBar
```

This will create an `src/themes/SearchBar` file in your project folder. Restart your dev server and edit the component, you will see that Ianaio uses your own `SearchBar` component now.
