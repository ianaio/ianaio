---
id: multi-instance
description: Use multiple docs plugin instances on a single Ianaio site.
slug: /docs-multi-instance
---

# Docs Multi-instance

The `@ianaio/plugin-content-docs` plugin can support [multi-instance](../../using-plugins.mdx#multi-instance-plugins-and-plugin-ids).
...
Sometimes you want a Ianaio site to host 2 distinct sets of documentation (or more).
...
If each documentation instance is very large, you should rather create 2 distinct Ianaio sites.
...
We use this pattern on the Ianaio website itself:
...
`@ianaio/preset-classic` already includes a docs plugin instance for you!
...
```bash npm2yarn
npm run ianaio docs:version 1.0.0
```

To version the non-default/community docs plugin instance:

```bash npm2yarn
npm run ianaio docs:version:community 1.0.0
```
