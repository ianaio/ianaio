...
Value should be one of `npm`, `yarn`, or `pnpm`. If it's not explicitly provided, Ianaio will infer one based on:

- The lockfile already present in the CWD (e.g. if you are setting up website in an existing project)
- The command used to invoke `create-ianaio` (e.g. `npm init`, `npx`, `yarn create`, etc.)
