...
This section is similar to [Styling and Layout](./styling-layout.mdx), but this time, we will write actual React code instead of playing with stylesheets. We will talk about a central concept in Ianaio: **swizzling**, which allows **deeper site customizations**.
...
Ianaio provides an convenient **interactive CLI** to swizzle components. You generally only need to remember the following command:
