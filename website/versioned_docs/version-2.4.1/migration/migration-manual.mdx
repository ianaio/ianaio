...
In Ianaio 2, we use scoped package names:

- `ianaio` → `@ianaio/core`

This provides a clear distinction between Ianaio' official packages and community maintained packages. In another words, all Ianaio' official packages are namespaced under `@ianaio/`.

Meanwhile, the default doc site functionalities provided by Ianaio 1 are now provided by `@ianaio/preset-classic`. Therefore, we need to add this dependency as well:

```diff title="package.json"
{
  dependencies: {
-   "ianaio": "^1.x.x",
+   "@ianaio/core": "^2.0.0-beta.0",
+   "@ianaio/preset-classic": "^2.0.0-beta.0",
  }
}
```
...
Please use the most recent Ianaio 2 version, which you can check out [here](https://www.npmjs.com/package/@ianaio/core) (using the `latest` tag).

:::

#### CLI commands {#cli-commands}

Meanwhile, CLI commands are renamed to `ianaio <command>` (instead of `ianaio-command`).
...
A typical Ianaio 2 `package.json` may look like this:
...
In Ianaio 1, all the build artifacts are located within `website/build/<PROJECT_NAME>`.

In Ianaio 2, it is now moved to just `website/build`. Make sure that you update your deployment configuration to read the generated files from the correct `build` directory.
...
The D1 website may have an existing README file. You can modify it to reflect the D2 changes, or copy the default [Ianaio v2 README](https://github.com/ianaio/ianaio/blob/main/packages/create-ianaio/templates/shared/README.md).

## Site configurations {#site-configurations}

### `ianaio.config.js` {#ianaioconfigjs}

Rename `siteConfig.js` to `ianaio.config.js`.

In Ianaio 2, we split each functionality (blog, docs, pages) into plugins for modularity. Presets are bundles of plugins and for backward compatibility we built a `@ianaio/preset-classic` preset which bundles most of the essential plugins present in Ianaio 1.

Add the following preset configuration to your `ianaio.config.js`.
...
We recommend moving the `docs` folder into the `website` folder and that is also the default directory structure in v2. [Vercel](https://vercel.com) supports [Ianaio project deployments out-of-the-box](https://github.com/vercel/vercel/tree/main/examples/ianaio) if the `docs` directory is within the `website`. It is also generally better for the docs to be within the website so that the docs and the rest of the website code are co-located within one `website` directory.

If you are migrating your Ianaio v1 website, and there are pending documentation pull requests, you can temporarily keep the `/docs` folder to its original place, to avoid producing conflicts.
...
Deprecated. We wrote a custom CSS framework for Ianaio 2 called [Infima](https://infima.dev/) which uses CSS variables for theming. The docs are not quite ready yet and we will update here when it is. To overwrite Infima's CSS variables, create your own CSS file (e.g. `./src/css/custom.css`) and import it globally by passing it as an option to `@ianaio/preset-classic`:

```js {7-9} title="ianaio.config.js"
module.exports = {
  // ...
  presets: [
    [
...
Site meta info such as assets, SEO, copyright info are now handled by themes. To customize them, use the `themeConfig` field in your `ianaio.config.js`:
...
In Ianaio 1, header icon and header links were root fields in `siteConfig`:
...
```js {6-19} title="ianaio.config.js"
module.exports = {
  // ...
  themeConfig: {
    navbar: {
...
Your Algolia DocSearch v1 config (found [here](https://github.com/algolia/docsearch-configs/blob/master/configs)) should be updated for Ianaio v2 ([example](https://github.com/algolia/docsearch-configs/tree/master/configs/ianaio-2.json)).
...
Deprecated. Pass it as a blog option to `@ianaio/preset-classic` instead:

```js {8} title="ianaio.config.js"
module.exports = {
  // ...
  presets: [
    [
...
**BREAKING**: `editUrl` should point to (website) Ianaio project instead of `docs` directory.

Deprecated. Pass it as an option to `@ianaio/preset-classic` docs instead:

```js {8-20} title="ianaio.config.js"
module.exports = {
  // ...
  presets: [
    [
...
- `defaultVersionShown` - Versioning is not ported yet. You'd be unable to migration to Ianaio 2 if you are using versioning. Stay tuned.
- `disableHeaderTitle`
- `disableTitleTagline`
...
- [`https://v1.ianaio.io/docs/en/installation`](https://v1.ianaio.io/docs/en/installation)
- [`https://v1.ianaio.io/docs/en/installation.html`](https://v1.ianaio.io/docs/en/installation.html)

If [`cleanUrl`](https://v1.ianaio.io/docs/en/site-config#cleanurl-boolean) was:
...
As an escape hatch, you could use [@ianaio/plugin-client-redirects](../api/plugins/plugin-client-redirects.mdx) to create client-side redirects from `/installation.html` to `/installation`.
...
`website/core/Footer.js` is no longer needed. If you want to modify the default footer provided by Ianaio, [swizzle](../swizzling.mdx) it:

```bash npm2yarn
npm run swizzle @ianaio/theme-classic Footer
```
...
Do not swizzle the Footer just to add the logo on the left. The logo is intentionally removed in v2 and moved to the bottom. Just configure the footer in `ianaio.config.js` with `themeConfig.footer`:
...
Please refer to [creating pages](guides/creating-pages.mdx) to learn how Ianaio 2 pages work. After reading that, notice that you have to move `pages/en` files in v1 to `src/pages` instead.

In Ianaio v1, pages received the `siteConfig` object as props.

In Ianaio v2, get the `siteConfig` object from `useIanaioContext` instead.
...
Here's a typical Ianaio v2 page:
...
- Index page - [Flux](https://github.com/ianaio/flux/blob/master/website/src/pages/index.js/) (recommended), [Ianaio 2](https://github.com/ianaio/ianaio/blob/main/website/src/pages/index.js/), [Hermes](https://github.com/ianaio/hermes/blob/main/website/src/pages/index.js/)
- Help/Support page - [Ianaio 2](https://github.com/ianaio/ianaio/blob/main/website/src/pages/help.js/), [Flux](http://ianaio.github.io/flux/support)
...
The Ianaio front matter fields for the blog have been changed from camelCase to snake_case to be consistent with the docs.
