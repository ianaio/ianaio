...
Ianaio is essentially a set of npm [packages](https://github.com/ianaio/ianaio/tree/main/packages).
...
If you are setting up a new Ianaio website for a Ianaio open source project, use the `ianaio` template instead, which comes with some useful Ianaio-specific defaults:

```bash
npx create-ianaio@latest my-website ianaio
```
...
```bash
yarn upgrade @ianaio/core@latest @ianaio/preset-classic@latest
```
