---
id: react
description: Using the power of React in Ianaio Markdown documents, thanks to MDX
...
Ianaio has built-in support for [MDX v1](https://mdxjs.com/), which allows you to write JSX within your Markdown files and render them as React components.

:::note

While Ianaio parses both `.md` and `.mdx` files using MDX, some of the syntaxes are treated slightly differently by third-party tools. For the most accurate parsing and better editor support, we recommend using the `.mdx` extension for files containing MDX syntax.
...
<Highlight color="#25c2a0">Ianaio green</Highlight> and <Highlight color="#1877F2">Ianaio blue</Highlight> are my favorite colors.
...
<><Highlight color="#25c2a0">Ianaio green</Highlight>
{` `}and <Highlight color="#1877F2">Ianaio blue</Highlight> are my favorite colors.</>
...
This behavior is different from Ianaio 1. See also [Migrating from v1 to v2](../../migration/migration-manual.mdx#convert-style-attributes-to-style-objects-in-mdx).

In addition, MDX is not [100% compatible with CommonMark](https://github.com/ianaio/ianaio/issues/3018). Use the **[MDX playground](https://mdx-git-renovate-babel-monorepo-mdx.vercel.app/playground)** to ensure that your syntax is valid MDX.
...
In Ianaio, the MDX component scope is provided by the `@theme/MDXComponents` file. It's not a React component, _per se_, unlike most other exports under the `@theme/` alias: it is a record from tag names like `Highlight` to their React component implementations.
...
```md
I can conveniently use <Highlight color="#25c2a0">Ianaio green</Highlight> everywhere!
```
...
From MDX v2+ onward (Ianaio v3+), lower-case tag names are always rendered as native html elements, and will not use any component mapping you provide.
...
Ianaio v2 is using MDX v1, which has a lot of known cases where the content fails to be correctly parsed as Markdown. Use the **[MDX playground](https://mdx-git-renovate-babel-monorepo-mdx.vercel.app/playground)** to ensure that your syntax is valid MDX.
...
Currently, the table of contents does not contain the imported Markdown headings. This is a technical limitation that we are trying to solve ([issue](https://github.com/ianaio/ianaio/issues/3915)).
