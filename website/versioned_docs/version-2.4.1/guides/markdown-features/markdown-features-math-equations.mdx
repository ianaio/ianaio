...
Use the exact same versions. The latest versions are incompatible with Ianaio 2.

:::

Import the plugins in `ianaio.config.js`:
...
Add them to your content plugin or preset options (usually `@ianaio/preset-classic` docs options):
...
The latest versions of `rehype-katex` (starting from `v6.0.0`) has moved to ES Modules, a new module system of JavaScript, which Ianaio doesn't officially support yet. However, it is possible to import `rehype-katex` dynamically, using an async config creator. Ianaio will call this creator function and wait for it to return the config object.
