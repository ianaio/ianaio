...
It is **easy to translate a Ianaio website** with its internationalization ([i18n](https://en.wikipedia.org/wiki/Internationalization_and_localization)) support.

## Goals {#goals}

It is important to understand the **design decisions** behind the Ianaio i18n support.

For more context, you can read the initial [RFC](https://github.com/ianaio/ianaio/issues/3317) and [PR](https://github.com/ianaio/ianaio/pull/3325).
...
- **Easy to use with [Crowdin](https://crowdin.com/)**: a lot of Ianaio v1 sites use Crowdin and should be able to migrate to v2
...
Overview of the workflow to create a translated Ianaio website:

1. **Configure**: declare the default locale and alternative locales in [`ianaio.config.js`](../api/ianaio.config.js.mdx#i18n)
...
This is the main content of your Ianaio website.
...
Some plugins may read from external data files that are localized as a whole. For example, the blog plugin uses an [`authors.yml`](../blog.mdx#global-authors) file that can be translated by creating a copy under `i18n/[locale]/ianaio-plugin-content-blog/authors.yml`.
...
Translating a very simple Ianaio site in French would lead to the following tree:
...
└── fr
    ...
    ├── ianaio-plugin-content-blog # translation data the blog plugin needs
    │   └── 2020-01-01-hello.md
    │
    ├── ianaio-plugin-content-docs # translation data the docs plugin needs
    │   ├── current
    │   │   ├── doc1.md
    │   │   └── doc2.mdx
    │   └── current.json
    │
    └── ianaio-theme-classic # translation data the classic theme needs
        ├── footer.json   # Text labels in your footer theme config
        └── navbar.json   # Text labels in your navbar theme config
```

The JSON files are initialized with the [`ianaio write-translations`](../cli.mdx#ianaio-write-translations-sitedir) CLI command. Each plugin sources its own translated content under the corresponding folder, while the `code.json` file defines all text labels used in the React code.
