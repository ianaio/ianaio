---
description: A Ianaio site is a pre-rendered single-page React application. You can style it the way you style React apps.
---
...
A Ianaio site is a single-page React application. You can style it the way you style React apps.
...
If you're using `@ianaio/preset-classic`, you can create your own CSS files (e.g. `/src/css/custom.css`) and import them globally by passing them as an option of the classic theme.
...
If you can't find a way to create a robust CSS selector, please [report your customization use-case](https://github.com/ianaio/ianaio/discussions/5468) and we will consider adding new class names.
...
```mdx-code-block
import ThemeClassNamesCode from '!!raw-loader!@site/../packages/ianaio-theme-common/src/utils/ThemeClassNames.ts';
...
`@ianaio/preset-classic` uses [Infima](https://infima.dev/) as the underlying styling framework. Infima provides a flexible layout and common UI components styling suitable for content-centric websites (blogs, documentation, landing pages). For more details, check out the [Infima website](https://infima.dev/).

When you scaffold your Ianaio project with `create-ianaio`, the website will be generated with basic Infima stylesheets and default styling. You can override Infima CSS variables globally.
...
It is possible to initialize the Ianaio theme directly from a `ianaio-theme` query string parameter.

Examples:

<IframeWindow url="/docs/?ianaio-theme=dark" />

<IframeWindow url="/docs/configuration?ianaio-theme=light" />

:::

### Data Attributes {#data-attributes}

It is possible to inject `<html>` data attributes with query string parameters following the `ianaio-data-<key>` pattern. This gives you the flexibility to style a page differently based on the query string.
...
html[data-red-border] div#__ianaio {
  border: red solid thick;
}
```

<IframeWindow url="/docs/?ianaio-data-navbar=false&ianaio-data-red-border" />

:::tip Iframe Mode

If you plan to embed some Ianaio pages on another site though an iframe, it can be useful to create an alternative display mode and use iframe urls such as `https://mysite.com/docs/myDoc?ianaio-data-mode=iframe`. It is your responsibility to provide the additional styles and decide which UI elements you want to keep or hide.

:::

### Mobile View {#mobile-view}

Ianaio uses `996px` as the cutoff between mobile screen width and desktop. If you want your layout to be different in the mobile view, you can use media queries.
...
CSS-in-JS support is a work in progress, so libs like MUI may have display quirks. [Welcoming PRs](https://github.com/ianaio/ianaio/issues/1640).

:::

## Sass/SCSS {#sassscss}

To use Sass/SCSS as your CSS preprocessor, install the unofficial Ianaio 2 plugin [`ianaio-plugin-sass`](https://github.com/rlamana/ianaio-plugin-sass). This plugin works for both global styles and the CSS modules approach:

1. Install [`ianaio-plugin-sass`](https://github.com/rlamana/ianaio-plugin-sass):

```bash npm2yarn
npm install --save ianaio-plugin-sass sass
```

2. Include the plugin in your `ianaio.config.js` file:

```js title="ianaio.config.js"
module.exports = {
  // ...
  // highlight-next-line
  plugins: ['ianaio-plugin-sass'],
  // ...
};
```
...
You can now set the `customCss` property of `@ianaio/preset-classic` to point to your Sass/SCSS file:
