...
This page explains how migrate a translated Ianaio v1 site to Ianaio v2.

## i18n differences {#i18n-differences}

Ianaio v2 i18n is conceptually quite similar to Ianaio v1 i18n with a few differences.
...
On Ianaio v2, localized content is generally found at `website/i18n/[locale]`.

Ianaio v2 is modular based on a plugin system, and each plugin is responsible to manage its own translations.

Each plugin has its own i18n subfolder, like: `website/i18n/fr/ianaio-plugin-content-blog`

### Updated translation APIs {#updated-translation-apis}

With Ianaio v1, you translate your pages with `<translate>`:
...
On Ianaio v2, you translate your pages with `<Translate>`
...
Ianaio v2 is using [MDX](https://mdxjs.com/) to parse Markdown files.

MDX compiles Markdown files to React components, is stricter than the Ianaio v1 parser, and will make your build fail on error instead of rendering some bad content.
...
There are **multiple possible strategies** to migrate a Ianaio v1 site using Crowdin, with different tradeoffs.
...
- Migrate your v1 Ianaio site to v2 without the translations
- Get familiar with the [new i18n system of Ianaio v2](../i18n/i18n-introduction.mdx) an
...
Don't try to migrate without understanding both Crowdin and Ianaio v2 i18n.
...
- Reconfigure `crowdin.yml` for Ianaio v2 according to the i18n docs
- Upload the Ianaio v2 source files with the Crowdin CLI to the new project
...
You will have to fix all the errors until your site builds. You can do that by modifying the translated MD files locally, and fix your site for one locale at a time using `ianaio build --locale fr`.
...
You could create a Crowdin branch for Ianaio v2, where you upload the v2 sources, and merge the Crowdin branch to main once ready.
...
Use the Crowdin CLI to download the v1 translated files, and put these translated files at the correct Ianaio v2 filesystem location.
