---
description: How to make your Ianaio site maximally search-engine-friendly.
sidebar_label: SEO
keywords:
  - seo
  - positioning
...
Ianaio supports search engine optimization in a variety of ways.
...
Ianaio adds some metadata out-of-the-box. For example, if you have configured [i18n](./i18n/i18n-introduction.mdx), you will get a [`hreflang`](https://developers.google.com/search/docs/advanced/crawling/localized-versions) alternate link.
...
Similar to [global metadata](#global-metadata), Ianaio also allows for the addition of meta-information to individual pages. Follow [this guide](./guides/markdown-features/markdown-features-head-metadata.mdx) for configuring the `<head>` tag. In short:
...
Ianaio automatically adds `description`, `title`, canonical URL links, and other useful metadata to each Markdown page. They are configurable through [front matter](./guides/markdown-features/markdown-features-intro.mdx#front-matter):
...
Prefer to use [front matter](./guides/markdown-features/markdown-features-intro.mdx#front-matter) for fields like `description` and `keywords`: Ianaio will automatically apply this to both `description` and `og:description`, while you would have to manually declare two metadata tags when using the `<head>` tag.
...
For JSX pages, you can use the Ianaio [`<Head>`](ianaio-core.mdx#head) component.
...
Ianaio is a static site generator—HTML files are statically generated for every URL route, which helps search engines discover your content more easily.
...
```md
![Ianaio banner](./assets/ianaio-asset-example-banner.png 'Image title')
```
...
To prevent your whole Ianaio site from being indexed, use the [`noIndex`](./api/ianaio.config.js.mdx#noIndex) site config. Some [hosting providers](./deployment.mdx) may also let you configure a `X-Robots-Tag: noindex` HTTP header (GitHub Pages does not support this).
...
For example, [`/examples/noIndex`](/examples/noIndex) is not included in the [Ianaio sitemap.xml file](pathname:///sitemap.xml) because it contains the following [page metadata](#single-page-metadata):
...
Ianaio uses your file names as links, but you can always change that using slugs, see this [tutorial](./guides/docs/docs-introduction.mdx#document-id) for more details.

## Structured content {#structured-content}

Search engines rely on the HTML markup such as `<h2>`, `<table>`, etc., to understand the structure of your webpage. When Ianaio renders your pages, semantic markup, e.g. `<aside>`, `<nav>`, `<main>`, are used to divide the different sections of the page, helping the search engine to locate parts like sidebar, navbar, and the main page content.
