...
import {useDocsPreferredVersion} from '@ianaio/theme-common';
import {useVersions} from '@ianaio/plugin-content-docs/client';
...
export function VersionsProvider({
  ...
  useEffect(() => {
    fetch('https://registry.npmjs.org/@ianaio/core')
      .then((res) => res.json())
      .then(
...
function GitBranchLink({branch}: {branch: string}): JSX.Element {
  return (
    <Link to={`https://github.com/ianaio/ianaio/tree/${branch}`}>
      <code>{branch}</code>
    </Link>
  );
}
...
export function StableMajorBranchLink(): JSX.Element {
  const majorVersionNumber = useStableMajorVersionNumber();
  const branch = `ianaio-v${majorVersionNumber}`;
...
export function PackageJSONDiff(): JSX.Element {
  ...
  return (
    <CodeBlock language="diff">
      {`-  "@ianaio/core": "^${stableVersion}",
-  "@ianaio/preset-classic": "^${stableVersion}",
+  "@ianaio/core": "${canaryVersion}",
+  "@ianaio/preset-classic": "${canaryVersion}",
`}
    </CodeBlock>
  );
}
