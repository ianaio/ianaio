    ...
    Joi.attempt(
      ...
      Joi.object<User>({
        ...
        preview: Joi.object({default: Joi.any()})
          ...
          .messages({
            'object.base':
              'The image should be hosted on Ianaio site, and not use remote HTTP or HTTPS URLs. It must be imported with require().',
          }),
        tags: Joi.array()
          .items(...TagList)
          .required(),
        source: Joi.string().allow(null).required().messages({
          'any.required':
            "The source attribute is required.\nIf your Ianaio site is not open-source, please make it explicit with 'source: null'.",
        }),
      }).unknown(false),
    );
