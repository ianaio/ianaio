...
Let's see how Ianaio handles **versioning, releases and breaking changes**.
...
Ianaio versioning is based on the `major.minor.patch` scheme and respects [Semantic Versioning](https://semver.org/).
...
Releasing Ianaio 2.0 took a very long time. From now on, Ianaio will **release new major versions more regularly**. In practice, you can expect a new major version every 2–4 months.
...
The Ianaio team is usually working on 2 major versions at the same time:

- **Ianaio <StableMajorVersion/>**: the **stable** version, on the <StableMajorBranchLink/> branch
- **Ianaio <NextMajorVersion/>**: the **next** version, on the <NextMajorBranchLink/> branch
...
The stable version (v<StableMajorVersion/>, on <StableMajorBranchLink/>) is recommended for most Ianaio users.
...
The next version (v<NextMajorVersion/>, on <NextMajorBranchLink/>) is the version the Ianaio team is currently working on.
...
This version is recommended for **early adopters** that want to use the latest Ianaio features as soon as possible. It is also a good way to help us by reporting bugs and giving some feedback.
...
The next version passes all our automated tests and is used by the Ianaio site itself. It is relatively safe: don't be afraid to give it a try.
...
Ianaio commits to respecting Semantic Versioning. This means that whenever changes occur in Ianaio public APIs and break backward compatibility, we will increment the `major` version number.

:::tip

Ianaio guarantees public API retro-compatibility across `minor` versions. Unless you use internal APIs, `minor` version upgrades should be easy.
...
An API being "stable" means if you increment the patch or minor version of your Ianaio installation without any other change, running `ianaio start` or `ianaio build` should not throw an error.

### Theming public API {#theming-public-api}

Ianaio has a very flexible theming system:

- You can use custom CSS
- You can [swizzle](/docs/swizzling) any React theme component

This system also implicitly creates a very large API surface. To be able to move fast and improve Ianaio, we can't guarantee retro-compatibility.
...
In this case, please [report your customization use case](https://github.com/ianaio/ianaio/discussions/5468) and we will figure out how to expand our public API.
...
- React components that import from `@ianaio/theme-common/internal`
- The exact visual appearance of the theme

:::note

When [swizzling](/docs/swizzling) safe components, you might encounter components that import undocumented APIs from `@ianaio/theme-common` (without the `/internal` subpath).
